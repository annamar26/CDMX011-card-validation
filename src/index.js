...
const logo = document.querySelector(".logo-bank")
...
submitButton.addEventListener("click", () => {
    ...
    if ((cardNumber.value.length < 16) & (cardNumber.value.length > 0)) {
        ...
        if (validator.getIssuer(cardNumber.value) === 'MasterCard') {
            logo.src = "mc.png"
        }
        if (validator.getIssuer(cardNumber.value) === 'Visa') {
            logo.src = "visa-logo.png"
        }
        console.log((validator.getIssuer(cardNumber.value)))
        console.log(logo)

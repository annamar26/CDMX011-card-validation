const validator = {
   maskify(cardNumber) {
    if (cardNumber.length > 4) {
      return cardNumber.replace( cardNumber.substring(0, cardNumber.length - 4), "#".repeat(cardNumber.length - 4)
      );
    }
    return cardNumber;
  },
...
if(i%2!==0){
 ...
 if(double.toString().length==2){
   ...
   array.push(digits.reduce((accumulator, currentValue) => parseInt(accumulator) + parseInt(currentValue)))
 }else{
   array.push(double)
 }
